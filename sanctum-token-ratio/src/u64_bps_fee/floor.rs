use crate::{AmtsAfterFee, FloorDiv, MathError, ReversibleFee, U64BpsFee, U64ValueRange};

impl ReversibleFee for FloorDiv<U64BpsFee> {
    fn apply(&self, amt_before_fee: u64) -> Result<AmtsAfterFee, MathError> {
        FloorDiv(self.0.try_to_u64_fee_ratio()?).apply(amt_before_fee)
    }

    fn reverse_from_amt_after_fee(&self, amt_after_fee: u64) -> Result<U64ValueRange, MathError> {
        FloorDiv(self.0.try_to_u64_fee_ratio()?).reverse_from_amt_after_fee(amt_after_fee)
    }

    fn reverse_from_fee_charged(&self, fee_charged: u64) -> Result<U64ValueRange, MathError> {
        FloorDiv(self.0.try_to_u64_fee_ratio()?).reverse_from_fee_charged(fee_charged)
    }
}
...
mod tests {
    ...
    prop_compose! {
        fn valid_fees()
            (fee in valid_bps_fee()) -> FloorDiv<U64BpsFee> {
                FloorDiv(fee)
            }
    }
    ...
    prop_compose! {
        fn valid_nonmax_fees()
            (fee in valid_nonmax_bps_fee()) -> FloorDiv<U64BpsFee> {
                FloorDiv(fee)
            }
    }
    ...
    prop_compose! {
        fn valid_nonzero_fees()
            (fee in valid_nonzero_bps_fee()) -> FloorDiv<U64BpsFee> {
                FloorDiv(fee)
            }
    }
    ...
    proptest! {
        ...
        fn zero_fee_apply_no_op(amt: u64) {
            let a = FloorDiv(U64BpsFee::ZERO).apply(amt).unwrap();
    ...
    proptest! {
        ...
        fn max_fee_apply_zero(amt: u64) {
            prop_assert_eq!(
                FloorDiv(U64BpsFee::MAX).apply(amt).unwrap(),
    ...
    proptest! {
        ...
        fn zero_fee_amt_after_fee_reverse_no_op(amt_after_fee: u64) {
            prop_assert_eq!(
                FloorDiv(U64BpsFee::ZERO).reverse_from_amt_after_fee(amt_after_fee).unwrap(),
    ...
    proptest! {
        ...
        fn max_fee_nonzero_amt_after_fee_reverse_err(non_zero_amt_after_fee in 1..=u64::MAX) {
            prop_assert_eq!(FloorDiv(U64BpsFee::MAX).reverse_from_amt_after_fee(non_zero_amt_after_fee).unwrap_err(), MathError);
        }
    }
    ...
    proptest! {
        ...
        fn zero_fee_nonzero_fee_charged_reverse_err(nonzero_fee_charged in 1..=u64::MAX) {
            prop_assert_eq!(FloorDiv(U64BpsFee::ZERO).reverse_from_fee_charged(nonzero_fee_charged).unwrap_err(), MathError);
        }
    }
    ...
    proptest! {
        ...
        fn max_fee_fee_charged_reverse_no_op(fee_charged: u64) {
            prop_assert_eq!(
                FloorDiv(U64BpsFee::MAX).reverse_from_fee_charged(fee_charged).unwrap(),
